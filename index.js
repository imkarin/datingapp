...
async function allUsers(req, res, next) {
	...
	await allUsersCollection.find({_id: mongo.ObjectId(req.session.user._id)}).forEach( function(userData){
...
function removeMovie(req, res) {
	...
	if (!req.session.user) {
		...
		allUsersCollection.updateOne({_id: mongo.ObjectId(userSessionID)}, {$pull: {movies: {title: selectedMovie}}}, (err) => {
			if (err) {
...
function changeImage(req, res) {
	allUsersCollection.updateOne({_id: mongo.ObjectId(req.session.user._id)}, { $set: {photo: req.file.originalname}}, (err) => { // set the photo-field in the database to the new uploaded image
		if (err) {
